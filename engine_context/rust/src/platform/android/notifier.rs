...
impl Notifier {
        ...
        F: Fn(&mut JNIEnv, &JObject) + 'static,
    {
        ...
        let notifier = &env.new_string("dev/irondash/engine_context/NativeNotifier")?;
        let notifier_class: JClass = env
            .call_method(
                class_loader.as_obj(),
                "loadClass",
                "(Ljava/lang/String;)Ljava/lang/Class;",
                &[JValueGen::Object(notifier)],
            )?
            .l()?
            .into();
    ...
    fn get_native_data(env: &mut jni::JNIEnv<'_>, obj: &JObject) -> Result<i64> {
        Ok(env.get_field(obj, "mNativeData", "J")?.j()?)
    }

    fn set_native_data(env: &mut jni::JNIEnv<'_>, obj: &JObject, data: i64) -> Result<()> {
